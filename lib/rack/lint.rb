  ...
  class Lint
    ...
    class Wrapper
      ...
      def check_environment(env)
        ...
        ## The CGI keys (named without a period) must have String values. If the string values for CGI keys contain non-ASCII characters, they should use ASCII-8BIT encoding.
        ...
        ##
        ## The environment is required to include these variables (adopted from {PEP 333}[https://peps.python.org/pep-0333/]), except when they'd be empty, but see below.

        ## <tt>REQUEST_METHOD</tt>:: The HTTP request method, such as "GET" or "POST". This cannot ever be an empty string, and so is always required.

        ## <tt>SCRIPT_NAME</tt>:: The initial portion of the request URL's "path" that corresponds to the application object, so that the application knows its virtual "location". This may be an empty string, if the application corresponds to the "root" of the server.
        ...
        ## <tt>QUERY_STRING</tt>:: The portion of the request URL that follows the <tt>?</tt>, if any. May be empty, but is always required!
        ...
        ## <tt>SERVER_PORT</tt>:: The port the server is running on. Should be specified if the server is running on a non-standard port.

        ## <tt>SERVER_PROTOCOL</tt>:: A string representing the HTTP version used for the request.
        ...
        ## <tt>rack.url_scheme</tt>:: The scheme of the incoming request, must be one of +http+, +https+, +ws+ or +wss+.

        ## <tt>rack.input</tt>:: See below, the input stream.

        ## <tt>rack.errors</tt>:: See below, the error stream.

        ## <tt>rack.hijack?</tt>:: See below, if present and true, indicates that the server supports partial hijacking.

        ## <tt>rack.hijack</tt>:: See below, if present, an object responding to +call+ that is used to perform a full hijack.

        ## <tt>rack.protocol</tt>:: An optional +Array+ of +String+, containing the protocols advertised by the client in the +upgrade+ header (HTTP/1) or the +:protocol+ pseudo-header (HTTP/2).
        ...
        ## <tt>rack.session</tt>:: A hash-like interface for storing request session data. The store must implement:
        ...
        ## <tt>rack.logger</tt>:: A common object interface for logging messages. The object must implement:
        ...
        ## The server or the application can store their own data in the environment, too.  The keys must contain at least one dot, and should be prefixed uniquely.  The prefix <tt>rack.</tt> is reserved for use with the Rack core distribution and other accepted specifications and must not be used otherwise.
        ##
        ...
        ## * One of <tt>SCRIPT_NAME</tt> or <tt>PATH_INFO</tt> must be set. <tt>PATH_INFO</tt> should be <tt>/</tt> if <tt>SCRIPT_NAME</tt> is empty.
      ...
      def call(stream)
        ...
        ##
        ## The +stream+ argument must implement: <tt>read, write, <<, flush, close, close_read, close_write, closed?</tt>
        ##
...
##
## == Thanks
## Some parts of this specification are adopted from {PEP 333 – Python Web Server Gateway Interface v1.0}[https://peps.python.org/pep-0333/] I'd like to thank everyone involved in that effort.
