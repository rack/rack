...
describe Rack::Response do
  ...
  it "can delete cookies with the same name from multiple domains" do
    ...
    response["Set-Cookie"].should.equal ["foo=bar; domain=sample.example.com", "foo=; domain=.example.com; max-age=0"].join("\n")
    response.delete_cookie "foo", :domain => "sample.example.com"
    response["Set-Cookie"].should.equal ["foo=; domain=.example.com; max-age=0",
                                         "foo=; domain=sample.example.com; max-age=0"].join("\n")
  end
